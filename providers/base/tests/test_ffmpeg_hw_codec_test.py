...
class TestCommandBuilders(unittest.TestCase):
    def test_decode_command(self):
        self.assertEqual(
            m.build_decode_command("in.mkv", "/dev/dri/renderD128"),
            [
                "ffmpeg",
                "-hwaccel",
                "vaapi",
...
class TestHwAccelerationUsed(unittest.TestCase):
    ...
    def test_entrypoint_too_far_after_profile(self):
        trace = "profile = 7\na\nb\nc\nentrypoint = 1\n"
        self.assertFalse(m.hw_acceleration_used(trace, "7", "1"))
...
class TestPerformTest(unittest.TestCase):
    ...
    def test_encode_uses_output_container(
        ...
        self.assertTrue(built[-1].endswith("out.mkv"))

    @patch("ffmpeg_hw_codec_test.run_ffmpeg")
    @patch("ffmpeg_hw_codec_test.os.path.exists", return_value=True)
    def test_workdir_removed(self, _exists, run_ffmpeg):
        run_ffmpeg.return_value = 0
        m.perform_test(self._args())
        workdir = os.path.dirname(run_ffmpeg.call_args[0][1])
        self.assertFalse(os.path.isdir(workdir))

    @patch("ffmpeg_hw_codec_test.run_ffmpeg")
    @patch("ffmpeg_hw_codec_test.os.path.exists", return_value=True)
    def test_workdir_removed_when_ffmpeg_raises(self, _exists, run_ffmpeg):
        run_ffmpeg.side_effect = OSError
...
class TestMain(unittest.TestCase):
    @patch("ffmpeg_hw_codec_test.perform_test", return_value=0)
    def test_main_parses_and_runs(self, perform_test):
        argv = ["decode", "in.mkv", "--profile", "7", "--entrypoint", "1"]
        self.assertEqual(m.main(argv), 0)
        self.assertEqual(perform_test.call_args[0][0].input, "in.mkv")
