...
class TestCmdGpu(unittest.TestCase):
    @patch("lzrt_host.has_level_zero_gpu", return_value=True)
    def test_returns_zero_when_gpu_found(self, _has):
        self.assertEqual(lzrt_host.cmd_gpu(), 0)

    @patch("lzrt_host.has_level_zero_gpu", return_value=False)
    def test_returns_one_when_no_gpu(self, _has):
        self.assertEqual(lzrt_host.cmd_gpu(), 1)


class TestMain(unittest.TestCase):
    @patch("lzrt_host.cmd_gpu", return_value=0)
    @patch("sys.argv", ["lzrt_host.py", "gpu"])
