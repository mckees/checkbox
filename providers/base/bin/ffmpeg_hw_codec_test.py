...
The media sample files are provided by the ``media-samples`` snap. The base
directory holding the samples can be overridden with the
``MEDIA_SAMPLES_PATH`` environment variable; it defaults to the snap mount
point ``/snap/media-samples/current/media-samples``.
...
def build_decode_command(input_file, device):
...
def perform_test(args):
    ...
    with tempfile.TemporaryDirectory(prefix="media_hw_codec_") as workdir:
        ...
        if args.operation == "decode":
            command = build_decode_command(input_file, args.device)
        else:
...
def main(argv=None):
    return perform_test(parse_args(argv))
